...
def run_dcf(
    ...
    output_folder: str = "dcf_output.csv",
    proj_name: str = ""
) -> tuple:
    ...
    """
    last_full_year = df_hist[df_hist["ttm"] == 0].index.max().year
    ...
    if forecast_horizon == 10:
        ...
        if df_plot["FCFF"][0] > 1000000:  # If first FCFF is large, assume it's in millions
            df_plot["Revenue"] /= 1000000
            df_plot["FCFF"] /= 1000000
            df_plot["EBIT"] /= 1000000
            df_plot["nopat"] /= 1000000
            df_plot["reinvest"] /= 1000000

        # Create sub-folder to store plot
        stc_projection_folder = os.path.join(output_folder, "stc_projection")
        os.makedirs(stc_projection_folder, exist_ok=True)
        # Plot EBIT, nopat, Reinvestments, and FCFF
        plt.figure(figsize=(10, 6))
        plt.plot(df_plot["Year"], df_plot["EBIT"], label="EBIT", marker='o', linestyle='-')
        plt.plot(df_plot["Year"], df_plot["Revenue"], label="Revenue", marker='^', linestyle='--')
        plt.plot(df_plot["Year"], df_plot["nopat"], label="NOPAT", marker='o', linestyle='-')
        plt.plot(df_plot["Year"], df_plot["reinvest"], label="Reinvestments", marker='x', linestyle='-')
        plt.plot(df_plot["Year"], df_plot["FCFF"], label="FCFF", marker='s', linestyle='-')
        plt.title(f"{p_method} - Projected EBIT, Reinvestments, and FCFF")

        # Annotate values above each dot
        for col, marker in zip(["EBIT", "reinvest", "FCFF"], ['o', 'x', 's']):
            for x, y in zip(df_plot["Year"], df_plot[col]):
                plt.annotate(f"{y:,.0f}", (x, y), textcoords="offset points", xytext=(0, 8), ha='center', fontsize=8)
        # Save plot to stc_projection_folder
        plot_path = os.path.join(stc_projection_folder, f"{p_method}_ebit_reinvest_fcff_{proj_name}.png")
        plt.legend()
        plt.tight_layout()
        plt.savefig(plot_path)
        plt.close()

    last_proj_fcf = df_proj_fcf["FCFF"].iloc[-1]


    # Discount cash flows (excluding terminal value for now)
    df_proj_fcf["discount_factor"] = 1 / ((1 + wacc) ** np.arange(1, len(df_proj_fcf["FCFF"]) + 1))
    df_proj_fcf["pv"] = df_proj_fcf["FCFF"] * df_proj_fcf["discount_factor"]
    pv_forecast_horizon = float(np.nansum(df_proj_fcf["pv"]))
    ...
    terminal_discount_factor = df_proj_fcf["discount_factor"].iloc[-1]
    pv_terminal = terminal_val * terminal_discount_factor

    # Build projected DataFrame with DCF metrics
    df_proj_fcf["Type"] = "Projected calendar year"
    ...
    df_proj_fcf = pd.concat([df_proj_fcf, pd.DataFrame([terminal_row])], ignore_index=True)

    # Build historical DataFrame for output
    df_hist_sub = df_hist.reset_index()
    df_hist_sub = df_hist_sub.rename(columns={"index": "Year"})
    df_hist_sub["growth_rate"] = df_hist_sub["Revenue"].pct_change(fill_method=None).fillna(0)
    ...
    full_df = pd.concat([df_hist_sub, df_proj_fcf], ignore_index=True)
