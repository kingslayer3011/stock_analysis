...
def main():
    ...
    # 7) Calculate and save growth stats
    print("Calculating growth statistics…")
    growth_data = calculate_growth_stats(df_hist)
    #growth_df = pd.DataFrame(list(growth_data.items()), columns=["variable", "cagr", "slope"]).set_index("variable")


    growth_csv_path = os.path.join(output_folder, "growth_rate.csv")
    try:
        growth_data.to_csv(growth_csv_path)
        print(f"Saved growth rates to '{growth_csv_path}'")
    except Exception as e:
        print(f"WARNING: Could not save growth_rate CSV: {e}", file=sys.stderr)

    # 8) Project future values and save CSV
    print("Projecting future values…")
    try:
        df_proj = project_values(ticker, final_params)
        ...
        print(f"WARNING: Could not save projected values CSV: {e}", file=sys.stderr)

    # 9) Run the main DCF calculation
    print("Running DCF calculation…")
    try:
        # NOTE: run_dcf signature is:
        #    run_dcf(df_hist, projection_years, terminal_growth, wacc, use_ebitda_base, fcf_to_ebitda_ratio)
    ...
    try:
        generate_pdf_report(
            ...
            dcf_df,
            dcf_valuation_info,
            sens1_df if use_ebitda_base else None,
            sens2_df,
            output_folder  # PDF goes into the same folder
        )
    except Exception as e:
