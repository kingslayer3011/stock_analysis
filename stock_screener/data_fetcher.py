...
def get_historical_data(
    ...
    non_na_delta_wc = df["Delta WC"].dropna()
    if not non_na_delta_wc.empty:
        ttm_row["Delta WC"] = non_na_delta_wc.iloc[-1]
    else:
        # Decide what to do if no values available, e.g. assign np.nan or 0 or skip
        ttm_row["Delta WC"] = np.nan
